...
//    @Published var cameraPosition: MapCameraPosition = .automatic
    ...
    func fetchData(settings: Settings, lat: Double, long: Double, completion: @escaping () -> Void = {}) {
        ...
        let url = endPoint(value:settings.selectedInBetween)+"observations/around-point/?days=\(14)&end_date=\(formatCurrentDate(value: settings.selectedDate))&lat=\(lat)&lng=\(long)&radius=\(settings.radius)&species_group=\(settings.selectedSpeciesGroupId)&min_rarity=\(settings.selectedRarity)"
